...
# --- JSON-escape a string (pure zsh builtins) ---
# Must escape all control chars U+0000-U+001F for valid JSON.
_rtlog_json_escape() {
    local s="$1"
...
_rtlog_precmd() {
    ...
    # Duration
    local dur=$(( EPOCHREALTIME - _rtlog_pending_start ))
    printf -v dur '%.1f' "$dur"
